...
void parse_data::check_argc(int argc, char *argv[]){
    ...
    inet_pton(AF_INET, argv[3],&this->victim_ip);
}
...
void parse_data::get_victim_mac(uint8_t mac[6]){
    memcpy(this->victim_mac,mac,6);
}
...
uint8_t* parse_data::using_victim_mac(){
    return this->victim_mac;
}
uint32_t *parse_data::using_victim_ip(){
    return &this->victim_ip;
}
